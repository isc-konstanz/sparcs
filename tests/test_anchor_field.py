...
def _obs(se_meas=0.8, variance=0.04, x_m=0.0, y_m=-0.3):
...
def test_single_sensor_reduces_to_constant_gain_blend():
    ...
    out = anchor_field(se, NEAR_FAR, [_obs(variance=0.04)], sigma_sys, 0.5, 0.5, SE_MIN, SE_MAX)
...
def test_zero_radius_is_a_noop():
    se = np.array([0.5, 0.5])
    out_h = anchor_field(se, NEAR_FAR, [_obs()], 0.1, 0.0, 0.5, SE_MIN, SE_MAX)
    out_v = anchor_field(se, NEAR_FAR, [_obs()], 0.1, 0.5, 0.0, SE_MIN, SE_MAX)
...
def test_zero_gain_is_a_noop():
    ...
    se = np.array([0.5, 0.5])
    out = anchor_field(se, NEAR_FAR, [_obs()], 0.0, 0.5, 0.5, SE_MIN, SE_MAX)
    assert np.array_equal(out, se)


def test_output_stays_within_physical_bounds():
    se = np.array([0.5, 0.5])
    high = anchor_field(se, NEAR_FAR, [_obs(se_meas=5.0)], 10.0, 0.5, 0.5, SE_MIN, SE_MAX)
    low = anchor_field(se, NEAR_FAR, [_obs(se_meas=-5.0)], 10.0, 0.5, 0.5, SE_MIN, SE_MAX)
...
def test_two_agreeing_sensors_pull_harder_without_overshoot():
    ...
    se = np.array([0.5, 0.5])
    one = anchor_field(se, NEAR_FAR, [_obs()], 0.1, 0.5, 0.5, SE_MIN, SE_MAX)
    two = anchor_field(se, NEAR_FAR, [_obs(), _obs()], 0.1, 0.5, 0.5, SE_MIN, SE_MAX)
...
def test_order_independent():
    se = np.array([0.5, 0.5])
    a = anchor_field(se, NEAR_FAR, [_obs(se_meas=0.8), _obs(se_meas=0.2)], 0.1, 0.5, 0.5, SE_MIN, SE_MAX)
    b = anchor_field(se, NEAR_FAR, [_obs(se_meas=0.2), _obs(se_meas=0.8)], 0.1, 0.5, 0.5, SE_MIN, SE_MAX)
