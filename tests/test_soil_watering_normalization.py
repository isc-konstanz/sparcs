...
``_compute_flux_rates`` only touches the flow column on ``et_data``, the
drip-line length, and the mesh config, so a bare instance built with
``object.__new__`` exercises it without the Component machinery; ``name`` is a
read-only property, so its backing attribute (``_name``) is set directly.
"""
...
def _bare_sim(drip_line_length_m: float) -> "SoilSimulation":
...
def _flow_m3s(sim: "SoilSimulation", flow_lpm: float = 60.0) -> float:
    et_data = pd.DataFrame(
        {SoilSimulation.IRRIGATION_FLOW_LPM: [flow_lpm]},
        index=pd.to_datetime(["2026-05-01 10:00"]),
    )
...
def test_default_length_keeps_per_metre_reading():
    ...
    sim = _bare_sim(drip_line_length_m=1.0)
    assert _flow_m3s(sim, flow_lpm=60.0) == pytest.approx(60.0 / 60_000.0)
...
def test_whole_field_flow_divided_by_drip_line_length():
    ...
    sim = _bare_sim(drip_line_length_m=1000.0)
    assert _flow_m3s(sim, flow_lpm=60.0) == pytest.approx(60.0 / 60_000.0 / 1000.0)


def test_missing_flow_column_reads_as_zero():
    """et_data without the flow column means "not watering", never an error."""
    sim = _bare_sim(drip_line_length_m=1.0)
    et_data = pd.DataFrame(index=pd.to_datetime(["2026-05-01 10:00"]))
    assert SoilSimulation._compute_flux_rates(sim, et_data, {}, 3600.0).flow_m3s == 0.0


def test_nan_flow_reads_as_zero():
    """A NaN flow row must never reach the PDE source (NULL = not watering)."""
    sim = _bare_sim(drip_line_length_m=1.0)
    assert _flow_m3s(sim, flow_lpm=float("nan")) == 0.0


def test_absurd_strip_flux_warns_once(caplog):
    """A strip flux far beyond drip rates warns once, not on every advance."""
    # 60 l/min over 1 m of line -> 72000 mm/h over the 0.05 m strip.
    sim = _bare_sim(drip_line_length_m=1.0)
    with caplog.at_level(logging.WARNING):
        _flow_m3s(sim, flow_lpm=60.0)
        _flow_m3s(sim, flow_lpm=60.0)
