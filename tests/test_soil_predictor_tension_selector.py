...
def test_peak_tension_is_worst_case_over_probes_and_time():
    ...
    and the whole horizon, ignoring probes outside decision_probes. The trajectory
    values are already water tension (hPa) at this point (converted upstream at the
    roll->publish boundary), so _peak_tension is a plain max -- no model."""
    trajectory = _trajectory(
        {
            "root_20": [50.0, 200.0, 600.0],  # peak tension 600 at t=2
            "root_40": [50.0, 50.0, 50.0],  # peak tension 50
            "surface": [1200.0, 1200.0, 1200.0],  # 1200 -- must be ignored (not a decision probe)
        }
    )

    peak = SoilPredictor._peak_tension(trajectory, decision_probes=["root_20", "root_40"])
...
def _fill_order_trajectories(peak_tensions: dict) -> dict:
    """One synthetic (already-tension) trajectory per rung, whose single value IS
    the rung's peak tension -- _peak_tension is now a plain max over tension."""
    trajectories = {}
...
def test_selector_fill_order_returns_first_feasible_rung():
    ...
    chosen, status = SoilPredictor._select(ladder, trajectories, ["root_20"], threshold_hpa, grid_mode="fill_order")
...
def test_selector_full_picks_least_total_minutes_among_feasible():
    ...
    chosen, status = SoilPredictor._select(ladder, trajectories, ["root_20"], threshold_hpa, grid_mode="full")
...
def test_trajectories_to_tension_converts_se_to_positive_hpa():
    """The roll->publish boundary converter maps per-probe Se trajectories to
    positive water tension (hPa): drier soil (lower Se) -> larger tension, out of
    the [0, 1] saturation range."""
    ...
    assert all(v > 1.0 for v in tension["root_20"])  # hPa magnitude, not Se in [0, 1]
    assert tension["root_20"][-1] > tension["root_20"][0]  # drier -> larger tension
...
def test_publish_results_publishes_predict_channel_values_as_is(monkeypatch):
    ...
    values are already water tension (hPa), converted upstream at the roll->publish
    boundary (see _trajectories_to_tension), so this method does not re-convert."""
    predictor = object.__new__(SoilPredictor)
    ...
    tension_traj = [10.0, 40.0, 90.0]  # tension rising as the soil dries
