...
from lories.components import Component, ComponentAccess
from lories.core import Configurations, ResourceError
from lories.util import to_timedelta, parse_freq
from lories.typing import Timestamp
from sparcs.components.control.predictive.model import Model
...
class Optimization(Component, ABC):
    ...
    def _configure_timing(self, configs: Configurations) -> None:
        ...
        if len(self.step_durations) == 0:
            raise ResourceError("No step durations configured, please check your timing configuration")
    ...
    def activate(self) -> None:
        ...
        if len(required_components) < len(self.required_components):
            pass
            #raise ResourceError(f"No required component found in system: {self.required_components}")
