# -*- coding: utf-8 -*-
"""
sparcs.components.solar.bifacial.irradiance
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
from pvlib.irradiance import get_extra_radiation, get_total_irradiance  # noqa: F401
...
def get_pvfactors_irradiance(
    ...
    def fn_build_report(pvarray):
        return {
            "total_inc_back": pvarray.ts_pvrows[index_observed_pvrow].back.get_param_weighted("qinc"),
            "total_inc_front": pvarray.ts_pvrows[index_observed_pvrow].front.get_param_weighted("qinc"),
            "total_abs_back": pvarray.ts_pvrows[index_observed_pvrow].back.get_param_weighted("qabs"),
            "total_abs_front": pvarray.ts_pvrows[index_observed_pvrow].front.get_param_weighted("qabs"),
        }
...
def _get_poa_irradiance(time, fn, *args, **kwargs):
    poa_front, poa_back = fn(*args, **kwargs)
    return _to_dict(time=time, poa_front=np.average(poa_front), poa_back=np.average(poa_back))


def _to_dict(**kwargs) -> dict:
